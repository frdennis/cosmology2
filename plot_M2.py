...
ax.plot(a[i], Xe[i], 'ko', label=r'Recombination (a = %.3e, $X_e$=%.3e)'%(a[i],Xe[i]))
ax.set_ylabel(r'$X_e$')
...
ax.plot(a[i], tau[i], 'ko', label=r'Recombination (a = %.3e, $\tau$=%.3e)'%(a[i],tau[i]))
ax.legend()
ax.grid()
...
ax[0].vlines(a[i], g.min(), g.max(), 'k', ls='--')
...
ax.hlines(Xe[j], a.min(), a.max(), 'k', ls='--', label=r'$X_e = 0.5$')
ax.grid()

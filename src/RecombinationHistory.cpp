...
int RecombinationHistory::rhs_peebles_ode(double x, const double *Xe, double *dXedx){
  ...
  // Peebles equations
  double n_H      = (1 - Yp)*3*pow(cosmo->H_of_x(0),2)*OmegaB0/(8*M_PI*G*m_H*pow(a,3.0)); // 1/m3
  double n1s      = (1. - X_e)*n_H; // 1/m3
  double lambda_a = (pow(hbar,-3)*pow(c,-3)) * H*pow(3*epsilon_0,3)/(pow(8*M_PI, 2)*n1s); // 1/s
  double phi2     = 0.448*log(epsilon_0/(Tb*k_b)); // dim. less
  double alpha2   = c * 64*M_PI/sqrt(27*M_PI)*pow(alpha/m_e,2)*sqrt(epsilon_0/(Tb*k_b))*phi2; // m3/s
  double beta     = (pow(hbar,-3)*pow(k_b, 3./2)) * alpha2*pow(m_e*Tb/(2*M_PI), 3./2) * exp(-(epsilon_0)/(Tb*k_b)); // 1/s
  double beta2    = (pow(hbar,-3)*pow(k_b, 3./2)) * alpha2*pow(m_e*Tb/(2*M_PI), 3./2) * exp(-(1./4.)*(epsilon_0)/(Tb*k_b)); // 1/s
  ...
  //double A  = c*8./3.*m_H/m_e*OmegaR0/OmegaB0/(cosmo->H_of_x(x))*n_H*X_e*sigma_T;

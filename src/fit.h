...
// * Check that line 86 is how you set up your background
// * Edit line 93 with the call to your luminosity function method (mine is called dL_of_x). As written here this assumes that the luminosity distance is returned in meters from this method
// * Include this file in Main.cpp: #include "fit.h"
// * Call the function below in main: fit_to_data("supernovadata.txt", "results.txt");
//
...
void fit_to_data(std::string supernovadata_filename, std::string outfile){

  // The number of parameters to fit and their indices in the array
  const int nparam = 3;
  ...
  auto comp_chi2 = [&](std::array<double, nparam> & parameters){
    ...
    double param_OmegaCDM = parameters[1] - param_OmegaB;
    ...
    for (size_t i = 0; i < z_arr.size(); i++){
      ...
      double L = cosmo.dL_of_x(x) / Gpc; // Luminosity function in units of Gpc
